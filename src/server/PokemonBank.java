...
public class PokemonBank {
	...
	public void handleClient() throws IOException, ClassNotFoundException {
		 ...
		 * - BankOperation
		 */
		ObjectInputStream input=new ObjectInputStream(socketduserveur.getInputStream());
		ObjectOutputStream output=new ObjectOutputStream(socketduserveur.getOutputStream());
		...
		while (running) {
			...
			case LIST:
				...
				if (this.pokemons.size() == 0) {
					 ...
					 */
                                        output.writeObject("No pokemon in database.");
                                        output.flush();
			...
			case STORE:
				...
				pokemons.add((Pokemon) input.readObject());
                                output.flush();
				 ...
				 */
                                ...
                                output.writeObject("Pokemon is safe.");
                                output.flush();
			...
			case CLOSE:
				 ...
				 */
                                System.out.println("Request: CLOSE");
                                output.writeObject("Ciao dear client, see you soon :)");
